# Local training pipeline for "Ricardo / ATML > 7.3 > Visual Regression (By Aditya Mangalampalli)"
...
3. Build the container:
    ...
    ```
    $ docker build -t custom-block-368559 .
    ```
...
4. Train your model:
    ...
    **macOS, Linux**
    ...
    ```
    $ docker run --rm -v $PWD:/scripts custom-block-368559 --data-directory data --out-directory out
    ```

    **Windows**

    ```
    $ docker run --rm -v "%cd%":/scripts custom-block-368559 --data-directory data --out-directory out
    ```
